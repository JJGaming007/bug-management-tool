...
export function DashboardStats({ stat }: { stat: 'total' | 'open' | 'resolved' | 'critical' }) {
  ...
  switch (stat) {
    case 'total':
      label = 'Total Bugs'
      value = bugs.length
      color = 'text-primary-600 ring-primary-100'
      break
    case 'open':
      label = 'Open Bugs'
  ...
  return (
    <div className="text-center">
      ...
      <span
        className={`mt-2 inline-block px-3 py-1 font-semibold bg-opacity-10 rounded-full ring ${color}`}
      >
        {value}
      </span>
    </div>
